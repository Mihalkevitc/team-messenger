...
// const fetchData = async () => { ... }
  ...
  const handleAddMember = async () => {
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/teams/${activeItem.id}/members`,
  ...
  const handleRemoveMember = async (userId) => {
    try {
      ...
      await axios.delete(
        `http://localhost:5000/api/teams/${activeItem.id}/members/${userId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
  ...
  const handleUpdateTeam = async () => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/teams/${activeItem.id}`,
        teamSettingsData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
